...
import com.gaozl.sqlite.utils.DateUtils;
...
public class SqlExecutor<T> {
    ...
    private final LinkedHashMap<StatementParts, String> queryParts = Maps.newLinkedHashMap();
    ...
    private final WhereExecutor<T> whereExecutor = new WhereExecutor<T>(this);
    private final JoinExecutor joinExecutor = new JoinExecutor();
    private final Map<String, Relationship> relationships = Maps.newHashMap();
     ...
     */
    public SqlExecutor<T> select(Class<T> clazz) throws DataConnException {
        reset();
        this.clazz = clazz;
    ...
    public SqlExecutor<T> update(Object databaseObject) throws DataConnException {
        reset();
     ...
     */
    @SuppressWarnings("unchecked")
    public SqlExecutor<T> save(T databaseObject) throws DataConnException {
        reset();
        clazz = (Class<T>) databaseObject.getClass();
        ...
        try {
            ...
            throw new DataConnException("Error running insert", e);
        }
        return this;
    }
     ...
     * @throws DataConnException
     */
    public SqlExecutor<T> delete(Class<T> clazz) throws DataConnException {
        reset();
        this.clazz = clazz;
     ...
     */
    @SuppressWarnings("unchecked")
    public SqlExecutor<T> delete(T databaseObject) throws DataConnException {
        reset();
        clazz = (Class<T>) databaseObject.getClass();
        sqlObject = databaseObject;
        queryParts.put(StatementParts.DELETE, DELETE);
     ...
     * @return A {@linkplain SqlExecutor} used for function chaining.
     */
    public SqlExecutor<T> join(Class<?> leftClazz, String leftField, Class<?> rightClazz, String rightField, JoinExecutor.JoinType joinType) {
...
//    assertEquals("select thing.* from thing join user on user.id = thing.userId", sql);
     ...
     */
    public SqlExecutor<T> from(Object object) throws DataConnException {
        String objectPk = this.getPkField(object.getClass());
        String thisFk = this.getFkField(object.getClass());

        if (objectPk == null) {
            throw new DataConnException("pkField on the target object couldn't be found... it's probably not declared on the object. To use this method the target object must have a PrimaryKey defined.");
        }
        if (thisFk == null) {
            throw new DataConnException(String.format("A OneToMany was found, however it wasn't a list of type %s", this.clazz.getName()));
        }
     ...
     * @throws DataConnException
     */
    public WhereExecutor<T> where(String field) throws DataConnException {
     ...
     * @throws DataConnException
     */
    public WhereExecutor<T> where(Class<?> clazz, String field) throws DataConnException {
        if (statementType == StatementType.UPDATE) {
            try {
                prepareUpdate(field);
            }
            catch (DataConnException e) {
                throw e;
            }
            catch (Exception e) {
                throw new DataConnException("Error running update", e);
            }
        }
        whereDefined = true;
    ...
    /**
     * Used to end a {@linkplain SqlStatement#update(Object) update}/{@linkplain SqlStatement(Object)
     * insert}/{@linkplain SqlStatement#delete(Class) delete} {@linkplain SqlStatement}.
     *
     * @throws DataConnException
     */
    public void execute() throws DataConnException {
        executeStatement();
    }
     ...
     * @throws DataConnException
     */
    public int getCount() throws DataConnException {
        ...
        try {
            ...
            throw new DataConnException("Could not process the count results of the query.", e);
        }
    }
     ...
     * @throws DataConnException
     */
    public List<T> getList() throws DataConnException {
        try {
            queryParts.put(StatementParts.SELECT, String.format(SELECT, clazz.getSimpleName().toLowerCase()));
            executeStatement();
            ...
            throw new DataConnException("An error occured when trying to get the list of " + clazz.getSimpleName() + " objects", e);
        }
    }
     ...
     * @throws DataConnException
     */
    public T getFirst() throws DataConnException {
     ...
     * @throws DataConnException
     */
    public List<Map<String, Object>> getColumns(ColumnExpression columnExpression) throws DataConnException {
        ...
        try {
            ...
            throw new DataConnException("Could not process the map results of the query.", e);
        }
    }


    String getQuery() throws DataConnException {
    ...
    private List<T> processResults() throws SQLException, InstantiationException, IllegalAccessException, SecurityException, IllegalArgumentException, NoSuchMethodException, InvocationTargetException, NoSuchFieldException, DataConnException {
    ...
    private void prepareUpdate(String field) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException, DataConnException, SecurityException, NoSuchMethodException {
        boolean first = true;
            ...
            if (fieldShouldBeInUpdateStatement) {
                try {
                    ...
                    if (classField.getType() == Boolean.class && clazz.getDeclaredMethod(methodName.replaceFirst("is", "get"), (Class<?>[]) null) != null) {
                        throw new DataConnException("boolean fields must name their fields isValue, not getValue");
                    }
    ...
    private void prepareInsert() throws IllegalArgumentException, IllegalAccessException, InvocationTargetException, SecurityException, NoSuchMethodException, DataConnException {
    ...
    private void replaceValues() throws SQLException, DataConnException {
//        logger.trace(String.format(getQuery().replaceAll("%", "%%").replaceAll("\\?", "%s"), values.toArray()));
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            if (object instanceof String) {
                ...
                String date = DateUtils.getDatabaseFormattedStringFromDate((java.util.Date) object);
    ...
    @SuppressWarnings("unchecked")
    private T createProxyObject(T object) throws DataConnException {
        relationships.putAll(findRelationships());

        if (relationships.size() > 0) {
            String objectPk = this.getPkField(this.clazz);
            if (objectPk == null) {
                throw new DataConnException("Error when mapping relationships. PkField on the target object couldn't be found... it's probably not declared on the object. To use this method the target object must have a PrimaryKey defined.");
            }
                ...
                proxyFactory.addAdvice(new MethodInterceptor() {
                    public Object invoke(MethodInvocation methodInvocation) throws Throwable {
                        ...
                        if (relationships.keySet().contains(fieldName)) {
                            ...
                            throw new DataConnException(String.format("No OneToMany relationship could be found on a member variable that corresponds to the method %s", methodInvocation.getMethod().getName()));
                        }
    ...
    private Map<String, Relationship> findRelationships() throws DataConnException {
        ...
        for (Field field : fields) {
            ...
            if (hasAnnotation) {
                ...
                if (targetClazz != List.class) {
                    throw new DataConnException(String.format("The return type of a OneToMany relationship must be a List Type for field %s", field.getName()));
                }
    ...
    private Object getPkValue(String pkField, Object object) throws DataConnException {
        try {
            return object.getClass().getMethod("get" + SqliteUtils.capitalize(pkField), (Class<?>[]) null).invoke(object, (Object[]) null);
        }
        catch (Exception e) {
            throw new DataConnException("Could not get pkValue", e);
        }
    }

    private String getFkField(Class<?> clazz) throws DataConnException {
        String fkField = null;
        Field[] fields = clazz.getDeclaredFields();
        ...
        if (!annotationFound) {
            throw new DataConnException(String.format("No OneToMany reationship could be found on the %s", clazz));
        }
        return fkField;
    }

    private void executeStatement() throws DataConnException {
        try {
            Connection connection = DataConnManager.getConnection();
            if (connection == null) {
                throw new DataConnException("Connection is not initialized");
            }
            ...
            if (needsAutoDefinedWhereStatement) {
                ...
                if (pkField == null) {
                    throw new DataConnException("pkField couldn't be found... it's probably not declared on the object.");
                }
                if (sqlObject == null) {
                    throw new DataConnException("An instance of the object " + clazz.getSimpleName() + " must be supplied to auto infer the upate/delete");
                }
                where(pkField);
...
//            logger.error(e.getErrorCode());
            if (e.getMessage().contains("PRIMARY KEY must be unique")) {
                throw new DataConnException("A @PrimaryKey needs to be defined on the class '" + this.clazz.getSimpleName() + "' , since there is a primary key in the database.", e);
            }
            throw new DataConnException("Error executing sql statement", e);
        }
        catch (Exception e) {
            throw new DataConnException("Error executing sql statement", e);
        }
    ...
    private void processColumn(Object object, String columnName, ResultSet resultSet) throws SQLException, SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException, NoSuchFieldException, DataConnException {
        Object value = null;
    ...
    private void addRelationshipForInsert(Relationship relationship) throws DataConnException {
        try {
            Method method = clazz.getDeclaredMethod(relationship.getterName(), (Class<?>[]) null);
            ...
            throw new DataConnException("Error adding relationship on object insertion", e);
        }
    }

    public void setupRelationshipForRelatedObject(Relationship relationship, Object object, String foreignKey) throws DataConnException {
        ...
        try {
            ...
            throw new DataConnException(String.format("Could not set the foreign key %s on the %s object", foreignKey, object.getClass()), e);
        }
        ...
        if (objectIsInDbAlready) {
            ...
            SqlStatement.save(object).execute();
        }
    }

    private void reset() throws DataConnException {
        try {
            if (statement != null) {
                statement.close();
            }
            ...
            throw new DataConnException("Failed to close sql statement", e);
        }
        queryParts.clear();
        values.clear();
        statement = null;
